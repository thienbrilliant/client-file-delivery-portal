...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const actor = await requireActor(); const { id } = await params; const parsed = schema.safeParse(await request.json());
    ...
    if (parsed.data.folderId) { const folder = await prisma.folder.findUnique({ where: { id: parsed.data.folderId }, select: { projectId: true } }); if (!folder || folder.projectId !== session.projectId) throw new AppError('INVALID_FOLDER', 'Thư mục không thuộc dự án này.'); }
    ...
    return Response.json({ data: { ...file, size: file.size.toString() }, error: null }, { status: 201 });
