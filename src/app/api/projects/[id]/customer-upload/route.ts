...
function validateName(value: string) {
  ...
  if (!name || name.length > 255 || name.includes('\0') || name === '.' || name === '..') {
    throw new AppError('INVALID_FILE', 'Tên file không hợp lệ.');
  }
  return name;
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!project || project.customerId !== actor.id || !project.customerUploadEnabled) {
      throw new AppError('FORBIDDEN', 'Dự án này chưa cho phép khách hàng gửi file.', 403);
    }

    const form = await request.formData();
    const entries = form.getAll('files').filter((value): value is File => value instanceof File);
    if (!entries.length) throw new AppError('INVALID_FILE', 'Chưa chọn file.');
    if (entries.length > MAX_FILES) throw new AppError('INVALID_FILE', `Chỉ được gửi tối đa ${MAX_FILES} file mỗi lần.`);

    const storage = createStorageProvider();
    const created = [];
    for (const file of entries) {
      if (file.size > MAX_FILE_SIZE) throw new AppError('FILE_TOO_LARGE', `File ${file.name} vượt quá giới hạn kích thước.`);
      const originalName = validateName(file.name);
      ...
      const storageKey = `projects/${projectId}/customer-uploads/${id}`;
      const buffer = Buffer.from(await file.arrayBuffer());
      const checksum = crypto.createHash('sha256').update(buffer).digest('hex');
      try {
        await storage.upload(buffer, storageKey, { 'content-type': file.type || 'application/octet-stream' });
        const item = await fileRepository.create({
          projectId,
          folderId: null,
          originalName,
          storedName: id,
        ...
        created.push({ ...item, size: item.size.toString() });
      } catch (error) {
