...
import { checkDistributedRateLimit } from '@/server/rate-limit-redis';
...
export async function POST(request: Request) {
  ...
  if (!(await checkDistributedRateLimit(`password-reset:${ip}`, 5, 15 * 60))) return errorResponse(new AppError('RATE_LIMITED', 'Bạn đã thử quá nhiều lần. Vui lòng thử lại sau.', 429));
  try {
    ...
    if (result) {
      ...
      if (user) await enqueueJob({ type: 'SEND_PASSWORD_RESET_EMAIL', idempotencyKey: `password-reset:${user.id}:${result.expiresAt.toISOString()}`, payload: { userId: user.id, email: result.email, name: user.name, sealedToken: sealSecret(result.token) } });
    }
