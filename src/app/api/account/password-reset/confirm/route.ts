...
export async function POST(request: Request) {
  ...
  if (!(await checkDistributedRateLimit(`password-reset-confirm:${ip}`, 8, 15 * 60))) return errorResponse(new AppError('RATE_LIMITED', 'Bạn đã thử quá nhiều lần. Vui lòng thử lại sau.', 429));
  try {
