...
export async function GET(request: Request) {
  try { await requireAdmin(); const url = new URL(request.url); const q = url.searchParams.get('q')?.trim() || undefined; const page = Math.max(1, Number(url.searchParams.get('page') || 1)); const take = Math.min(50, Math.max(1, Number(url.searchParams.get('pageSize') || 20))); const [items, total] = await customerRepository.list({ q, skip: (page - 1) * take, take }); return Response.json({ data: { items, total, page, pageSize: take }, error: null }); } catch (error) { return errorResponse(error); }
}
