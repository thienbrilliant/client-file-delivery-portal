...
import { ArrowUpRight, Bell, Download, FileText, FolderKanban, PackageCheck } from 'lucide-react';
...
export default async function PortalPage() {
  ...
  return <div className="space-y-7">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[
      ['Dự án đang có', projects.length, FolderKanban], ['Bàn giao gần đây', deliveries.length, PackageCheck], ['File của bạn', fileCount, FileText], ['Chưa đọc', unreadCount, Bell],
    ].map(([label, value, Icon]) => <div key={String(label)} className="surface rounded-[10px] p-4"><div className="flex items-center justify-between"><span className="text-sm text-[var(--muted)]">{label}</span><Icon className="h-4 w-4 text-[var(--muted)]" /></div><p className="mt-4 text-xl font-semibold">{value}</p></div>)}</section>
