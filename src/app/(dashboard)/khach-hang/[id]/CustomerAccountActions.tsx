...
export function CustomerAccountActions({ id, status }: { id: string; status: string }) {
  ...
  async function run(action: string) {
    const labels: Record<string, string> = { suspend: 'Tạm khóa tài khoản này?', disable: 'Vô hiệu hóa tài khoản này?', 'revoke-sessions': 'Đăng xuất khỏi tất cả thiết bị?', reactivate: 'Kích hoạt lại tài khoản này?' };
    if (labels[action] && !window.confirm(labels[action])) return;
    await request(`/api/customers/${id}/account`, { action });
  }
