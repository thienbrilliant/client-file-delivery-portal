...
export default function ProjectDetail({params}:{params:Promise<{id:string}>}) {
  ...
  return <div className="space-y-6"><div><Link href="/du-an" className="inline-flex items-center gap-2 text-sm text-[var(--muted)] hover:text-[var(--foreground)]"><ArrowLeft className="h-4 w-4"/>Dự án</Link><div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-semibold tracking-tight">{project.name}</h1><p className="mt-1 text-sm text-[var(--muted)]">{project.customer.name||project.customer.email} · {project.status}</p></div><div className="flex gap-2"><input ref={input} type="file" multiple hidden onChange={upload}/><Button disabled={busy} onClick={()=>input.current?.click()}><Upload className="h-4 w-4"/>{busy?'Đang tải...':'Tải file lên'}</Button><Button variant="secondary" onClick={async()=>{const n=prompt('Tên thư mục');if(!n)return;const response=await fetch('/api/folders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectId:id,parentId:folderId,name:n})});if(response.ok)await load()}}><FolderPlus className="h-4 w-4"/>Thư mục mới</Button></div></div></div><div className="surface rounded-[10px] p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="text-sm text-[var(--muted)]">{project.name} / {folderId?'Thư mục hiện tại':'Gốc'}</div><div className="w-full sm:max-w-xs"><Input placeholder="Tìm file..." value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&void load()}/></div></div><div className="mt-5 space-y-1">{folderId&&<button onClick={()=>setFolderId(null)} className="flex w-full items-center gap-3 rounded-[8px] px-3 py-2 text-left text-sm hover:bg-[var(--surface-muted)]"><Folder className="h-4 w-4"/>Quay lại</button>}{folders.map(f=><button key={f.id} onClick={()=>setFolderId(f.id)} className="flex w-full items-center gap-3 rounded-[8px] px-3 py-2 text-left text-sm hover:bg-[var(--surface-muted)]"><Folder className="h-4 w-4"/><span className="font-medium">{f.name}</span><span className="ml-auto text-xs text-[var(--muted)]">{f._count.files} file</span></button>)}{files.map(f=><div key={f.id} className="flex items-center gap-3 rounded-[8px] px-3 py-2 text-sm hover:bg-[var(--surface-muted)]"><File className="h-4 w-4 text-[var(--muted)]"/><span className="min-w-0 flex-1 truncate">{f.originalName}</span><span className="hidden text-xs text-[var(--muted)] sm:block">{(Number(f.size)/1024/1024).toFixed(1)} MB</span><a className="p-2 text-[var(--muted)] hover:text-[var(--foreground)]" href={`/api/files/${f.id}`} aria-label={`Tải xuống ${f.originalName}`}><Download className="h-4 w-4"/></a><button aria-label={`Xóa ${f.originalName}`} className="p-2 text-[var(--muted)] hover:text-[var(--destructive)]" onClick={async()=>{if(confirm(`Xóa ${f.originalName}?`)){const response=await fetch(`/api/files/${f.id}`,{method:'DELETE'});if(response.ok)await load()}}}><Trash2 className="h-4 w-4"/></button></div>)}{!folders.length&&!files.length&&<div className="py-14 text-center"><p className="font-medium">Chưa có file nào</p><p className="mt-1 text-sm text-[var(--muted)]">Tải file đầu tiên lên để bắt đầu quản lý tài liệu.</p></div>}</div></div></div>;
}
