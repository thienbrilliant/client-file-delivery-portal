      ...
      <section className="relative">
        ...
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
          <Link href="/" className="flex items-center gap-3 font-semibold tracking-tight">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--primary)] text-white shadow-lg shadow-emerald-950/10"><FolderKanban className="h-4 w-4" /></span>
            <span>{siteConfig.name}</span>
          </Link>
          <div className="hidden items-center gap-7 text-sm text-[var(--muted)] md:flex">
            <a href="#tinh-nang" className="hover:text-[var(--foreground)]">Tính năng</a>
            <a href="#quy-trinh" className="hover:text-[var(--foreground)]">Quy trình</a>
            <a href="#bat-dau" className="hover:text-[var(--foreground)]">Bắt đầu</a>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/dang-nhap" className="rounded-xl px-3 py-2 text-sm font-medium hover:bg-white/70">Đăng nhập</Link>
            <Link href="/dang-ky" className="hidden rounded-xl bg-[var(--foreground)] px-4 py-2 text-sm font-semibold text-white shadow-sm sm:inline-flex">Dùng thử miễn phí</Link>
          </div>
        </nav>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 pb-20 pt-10 sm:px-8 lg:grid-cols-[1.02fr_.98fr] lg:pb-28 lg:pt-16">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-white/75 px-3 py-1.5 text-xs font-medium text-[var(--primary)] shadow-sm backdrop-blur"><Sparkles className="h-3.5 w-3.5" /> File delivery, nhưng đỡ đau đầu hơn</div>
            <h1 className="mt-6 max-w-3xl text-5xl font-semibold tracking-[-.045em] text-[var(--foreground)] sm:text-6xl lg:text-[72px] lg:leading-[1.02]">Bàn giao file <span className="text-[var(--primary)]">đẹp, nhanh, có kiểm soát.</span></h1>
            <p className="mt-6 max-w-xl text-base leading-7 text-[var(--muted)] sm:text-lg">Một workspace dành cho team dịch vụ sáng tạo: quản lý khách hàng, dự án, file và bàn giao trong cùng một nơi.</p>
            <div id="bat-dau" className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href="/dang-ky" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-emerald-950/15 transition-transform hover:-translate-y-0.5">Tạo tài khoản miễn phí <ArrowRight className="h-4 w-4" /></Link>
              <Link href="/dang-nhap" className="inline-flex items-center justify-center rounded-xl border border-[var(--border)] bg-white/70 px-5 py-3.5 text-sm font-semibold hover:bg-white">Tôi đã có tài khoản</Link>
            </div>
            <div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-xs text-[var(--muted)]"><span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[var(--success)]" /> Không cần thẻ</span><span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[var(--success)]" /> Workspace riêng</span><span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[var(--success)]" /> Google sign-in</span></div>
          </div>
          <div className="relative lg:pl-4">
            <div className="absolute -inset-8 -z-10 rounded-[40px] bg-[radial-gradient(circle,rgba(47,93,80,.16),transparent_62%)] blur-2xl" />
            <div className="overflow-hidden rounded-[28px] border border-white/70 bg-white/70 p-2 shadow-[0_28px_80px_rgba(35,55,45,.16)] backdrop-blur">
              <img src="/portal-preview.svg" alt="Minh họa workspace quản lý file và bàn giao" className="block w-full rounded-[22px]" />
            </div>
            <div className="absolute -bottom-5 -left-3 rounded-2xl border border-white bg-white/90 px-4 py-3 shadow-xl backdrop-blur sm:-left-8"><p className="text-[11px] text-[var(--muted)]">Trạng thái bàn giao</p><p className="mt-1 flex items-center gap-2 text-sm font-semibold"><span className="h-2 w-2 rounded-full bg-[var(--success)]" /> Sẵn sàng cho khách</p></div>
          </div>
        </div>
      </section>

      <section id="tinh-nang" className="border-y border-[var(--border)] bg-white/55 py-20 sm:py-24">
        <div className="mx-auto max-w-7xl px-5 sm:px-8"><div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[.16em] text-[var(--primary)]">Một nơi cho toàn bộ flow</p><h2 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Ít tab hơn. Ít thất lạc hơn.</h2><p className="mt-4 text-sm leading-6 text-[var(--muted)] sm:text-base">Thiết kế để thao tác hằng ngày nhanh: nhìn là hiểu, bấm là tới đúng chỗ, và luôn biết điều gì cần làm tiếp.</p></div><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{features.map(({ icon: Icon, title, text }) => <article key={title} className="group rounded-2xl border border-[var(--border)] bg-white p-5 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-black/5"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--primary-soft)] text-[var(--primary)]"><Icon className="h-5 w-5" /></div><h3 className="mt-5 text-sm font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{text}</p></article>)}</div></div>
      </section>
