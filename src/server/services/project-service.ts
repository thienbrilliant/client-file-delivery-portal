...
export const projectService = {
  async create(actor: Actor, input: { customerId: string; name: string; description?: string; status?: 'DRAFT'|'IN_PROGRESS'|'READY'|'ARCHIVED' }) {
  ...
  async update(actor: Actor, projectId: string, input: { name: string; description?: string | null; status?: 'DRAFT'|'IN_PROGRESS'|'READY'|'ARCHIVED' }) {
    ...
    const item = await projectRepository.update(projectId, input); await logActivity({ userId: actor.id, projectId, action: 'PROJECT_UPDATED', metadata: { name: item.name, status: item.status } }); return item;
  },
};
