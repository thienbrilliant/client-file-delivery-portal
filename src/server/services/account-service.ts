...
export const accountService = {
  async createInvitation(userId: string) {
    ...
    const token = createSecureToken(); const expiresAt = new Date(Date.now() + INVITATION_TTL_MS);
    await prisma.$transaction(async tx => { await tx.accountInvitation.updateMany({ where: { userId, usedAt: null }, data: { usedAt: new Date() } }); await tx.accountInvitation.create({ data: { userId, tokenHash: hashToken(token), expiresAt } }); await tx.user.update({ where: { id: userId }, data: { status: 'INVITED', passwordHash: null, sessionVersion: { increment: 1 } } }); });
  ...
  async activateInvitation(token: string, passwordHash: string) {
    ...
    await prisma.$transaction([prisma.accountInvitation.update({ where: { id: invitation.id }, data: { usedAt: new Date() } }), prisma.user.update({ where: { id: invitation.userId }, data: { passwordHash, status: 'ACTIVE', emailVerified: new Date(), sessionVersion: { increment: 1 } } }), prisma.activityLog.create({ data: { userId: invitation.userId, action: 'CUSTOMER_ACTIVATED' } })]);
    return { id: invitation.user.id, email: invitation.user.email };
  },
  async requestPasswordReset(email: string) {
    const user = await prisma.user.findUnique({ where: { email: email.toLowerCase() }, select: { id: true, email: true, name: true, status: true, deletedAt: true } });
    if (!user || user.deletedAt || !['ACTIVE', 'SUSPENDED'].includes(user.status)) return null;
    const token = createSecureToken(); const expiresAt = new Date(Date.now() + RESET_TTL_MS);
    await prisma.$transaction(async tx => { await tx.passwordResetToken.updateMany({ where: { userId: user.id, usedAt: null }, data: { usedAt: new Date() } }); await tx.passwordResetToken.create({ data: { userId: user.id, tokenHash: hashToken(token), expiresAt } }); await tx.user.update({ where: { id: user.id }, data: { sessionVersion: { increment: 1 } } }); await tx.activityLog.create({ data: { userId: user.id, action: 'CUSTOMER_PASSWORD_RESET_REQUESTED' } }); });
  ...
  async resetPassword(token: string, passwordHash: string) {
    ...
    await prisma.$transaction([prisma.passwordResetToken.update({ where: { id: item.id }, data: { usedAt: new Date() } }), prisma.user.update({ where: { id: item.userId }, data: { passwordHash, sessionVersion: { increment: 1 } } }), prisma.activityLog.create({ data: { userId: item.userId, action: 'CUSTOMER_PASSWORD_RESET' } })]);
  },
  async setStatus(userId: string, status: 'SUSPENDED' | 'ACTIVE' | 'DISABLED', actorId: string) {
    ...
    const user = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, role: true, deletedAt: true } });
    if (!user || user.role !== 'CUSTOMER') throw new AppError('CUSTOMER_NOT_FOUND', 'Không tìm thấy khách hàng.', 404);
    if (user.deletedAt && status === 'ACTIVE') throw new AppError('FORBIDDEN', 'Tài khoản đã được xóa mềm và không thể kích hoạt lại bằng thao tác thông thường.', 409);
  ...
  async revokeSessions(userId: string, actorId: string) {
    ...
    return prisma.$transaction(async tx => { const user = await tx.user.update({ where: { id: userId }, data: { sessionVersion: { increment: 1 } }, select: { sessionVersion: true } }); const sessions = await tx.session.deleteMany({ where: { userId } }); await tx.activityLog.create({ data: { userId, action: 'CUSTOMER_SESSIONS_REVOKED', metadata: { actorId } } }); return { deletedSessions: sessions.count, sessionVersion: user.sessionVersion }; });
  },
  async softDelete(userId: string, actorId: string) {
    ...
    if (!user || user.role !== 'CUSTOMER') throw new AppError('CUSTOMER_NOT_FOUND', 'Không tìm thấy khách hàng.', 404);
    await prisma.$transaction(async tx => { await tx.user.update({ where: { id: userId }, data: { status: 'DISABLED', deletedAt: new Date(), sessionVersion: { increment: 1 } } }); await tx.session.deleteMany({ where: { userId } }); await tx.activityLog.create({ data: { userId, action: 'CUSTOMER_DELETE_REQUESTED', metadata: { actorId } } }); });
